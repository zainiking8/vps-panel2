...
| # | Tool | What it does |
|---|---|---|
| 0 | **AUTO FIX EVERYTHING** | **Auto-installs missing packages (nginx/dropbear), fixes DNS/SSH/nginx, resets panel creds, re-issues SSL, and restarts all services** |
...
- **dropbear failed / nginx inactive / service missing?** → **Option 0** auto-installs missing packages
...
### `dropbear failed` or `nginx inactive`

**Cause**: The required package (`dropbear` or `nginx`) was not installed.

**Fix**:
1. **Option 12 → Option 0 (AUTO FIX EVERYTHING)** — it auto-installs missing packages and restarts services
2. If still failing, run manually:
   ```bash
   apt-get update && apt-get install -y dropbear openssh-server nginx
   ```
3. Then run `./menu.sh` → Option 12 → Option 0 again
