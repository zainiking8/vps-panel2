    ...
    if os.path.exists(CONFIG_FILE):
        for line in open(CONFIG_FILE):
            ...
            if "=" in line:
                k, v = line.split("=", 1)
                v = v.strip()
                # Strip surrounding quotes if present (config file uses PANEL_PORT="8443")
...
def build_stream_grpc_nontls(domain, service="tunnel"):
    return {
        "network": "grpc",
        "security": "none",
        "grpcSettings": {
...
def build_stream_ws_nontls(path="/tunnel"):
    return {
        "network": "ws",
        "security": "none",
        "wsSettings": {
...
def gen_vless_link(client_uuid, domain, port, transport, sni, service_or_path, remark, tls=True):
    params = []
    if transport == "grpc":
...
def gen_vmess_link(client_uuid, domain, port, transport, sni, service_or_path, remark, tls=True):
    obj = {
        "v": "2",
        "ps": remark,
        "add": domain,
...
def gen_trojan_link(password, domain, port, transport, sni, service_or_path, remark, tls=True):
    params = []
    if transport == "grpc":
        params.append("type=grpc")
        params.append("serviceName=" + service_or_path)
...
def do_create(protocol, transport, domain, cert, key, port_choice, base, web_base, cookie, tls=True):
    ...
    service = "tunnel"
    path = "/tunnel"
    sec_label = "TLS" if tls else "None-TLS"

    if protocol == "vless":
        settings = build_vless_settings(client_uuid, remark, ip_limit, total_gb, expiry_time)
        if transport == "grpc":
            stream = build_stream_grpc(domain, cert, key, service) if tls else build_stream_grpc_nontls(domain, service)
            link = gen_vless_link(client_uuid, domain, port_choice, "grpc", domain, service, remark, tls)
        else:
            stream = build_stream_ws(domain, cert, key, path) if tls else build_stream_ws_nontls(path)
            link = gen_vless_link(client_uuid, domain, port_choice, "ws", domain, path, remark, tls)
    elif protocol == "vmess":
        settings = build_vmess_settings(client_uuid, remark, ip_limit, total_gb, expiry_time)
        if transport == "grpc":
            stream = build_stream_grpc(domain, cert, key, service) if tls else build_stream_grpc_nontls(domain, service)
            link = gen_vmess_link(client_uuid, domain, port_choice, "grpc", domain, service, remark, tls)
        else:
            stream = build_stream_ws(domain, cert, key, path) if tls else build_stream_ws_nontls(path)
            link = gen_vmess_link(client_uuid, domain, port_choice, "ws", domain, path, remark, tls)
    elif protocol == "trojan":
        trojan_pass = gen_uuid()
        settings = build_trojan_settings(trojan_pass, remark, ip_limit, total_gb, expiry_time)
        if transport == "grpc":
            stream = build_stream_grpc(domain, cert, key, service) if tls else build_stream_grpc_nontls(domain, service)
            link = gen_trojan_link(trojan_pass, domain, port_choice, "grpc", domain, service, remark, tls)
        else:
            stream = build_stream_ws(domain, cert, key, path) if tls else build_stream_ws_nontls(path)
            link = gen_trojan_link(trojan_pass, domain, port_choice, "ws", domain, path, remark, tls)
    else:
...
def main():
    if len(sys.argv) < 2:
        ...
        print("Commands (TLS): create-vless-grpc, create-vless-ws, create-vmess-grpc,")
        print("                create-vmess-ws, create-trojan-grpc, create-trojan-ws,")
        print("Commands (None-TLS): create-vless-grpc-nontls, create-vless-ws-nontls,")
        print("                    create-vmess-grpc-nontls, create-vmess-ws-nontls,")
        print("                    create-trojan-grpc-nontls, create-trojan-ws-nontls,")
    ...
    if cmd.startswith("create-"):
        ...
        is_nontls = len(parts) > 3 and parts[3] == "nontls"
        ...
        port_choice = pick_port(80 if is_nontls else 443)
        sec_label = "None-TLS" if is_nontls else "TLS"
        print(f"\n  Selected: {protocol.upper()} + {transport.upper()} + {sec_label}")
        print(f"  Auto Port: {port_choice}")
        do_create(protocol, transport, domain, cert, key, port_choice, base, web_base, cookie, tls=not is_nontls)
    elif cmd == "list":
...
xui_account_menu() {
    ...
    while true; do
        ...
        echo -e " ${CYAN}--------------------${NC}"
        echo -e " ${GREEN}VLESS / VMess / Trojan (None-TLS):${NC}"
        echo -e "  7) VLESS + gRPC (None-TLS)"
        echo -e "  8) VLESS + WebSocket (None-TLS)"
        echo -e "  9) VMess + gRPC (None-TLS)"
        echo -e " 10) VMess + WebSocket (None-TLS)"
        echo -e " 11) Trojan + gRPC (None-TLS)"
        echo -e " 12) Trojan + WebSocket (None-TLS)"
        echo -e " ${CYAN}--------------------${NC}"
        ...
        case $x_opt in
            ...
            7) python3 /usr/local/bin/xui-helper.py create-vless-grpc-nontls ;;
            8) python3 /usr/local/bin/xui-helper.py create-vless-ws-nontls ;;
            9) python3 /usr/local/bin/xui-helper.py create-vmess-grpc-nontls ;;
            10) python3 /usr/local/bin/xui-helper.py create-vmess-ws-nontls ;;
            11) python3 /usr/local/bin/xui-helper.py create-trojan-grpc-nontls ;;
            12) python3 /usr/local/bin/xui-helper.py create-trojan-ws-nontls ;;
