...
uninstall_panel() {
    ...
    echo -e "\n${BLUE}[1/10] Stopping & disabling ALL services...${NC}"
    ...
    echo -e "${YELLOW}Packages purge kar rahe hain (nginx, dropbear, certbot)...${NC}"
    apt remove --purge -y nginx nginx-common dropbear certbot 2>/dev/null
    ...
    echo -e "  - Nginx, Dropbear, Certbot  [PURGED]"
...
install_3xui_panel() {
    ...
    if ! command -v dropbear &>/dev/null; then
        echo -e "${YELLOW}SSH system install nahi hai. Auto-install ho raha hai...${NC}"
        install_all_components
    else
        echo -e "${GREEN}[OK] SSH system already installed (Dropbear).${NC}"
    fi
    ...
    if [[ -f "/root/cert/${current_dom}/fullchain.pem" ]]; then
        echo -e "${GREEN}[OK] SSL cert already exists.${NC}"
    else
        echo -e "${YELLOW}Port 80 free karke SSL cert ban raha hai (acme.sh)...${NC}"
        systemctl stop nginx 2>/dev/null
        systemctl stop x-ui 2>/dev/null
        if [[ ! -d /root/.acme.sh ]]; then
            echo -e "${YELLOW}acme.sh install ho raha hai...${NC}"
            if ! curl -4 -sL https://get.acme.sh | sh >/dev/null 2>&1; then
                echo -e "${RED}[ERROR] acme.sh install nahi hua. Internet/DNS check karein.${NC}"
        ...
        if [[ -f /root/.acme.sh/acme.sh ]]; then
            ...
            /root/.acme.sh/acme.sh --issue -d "$current_dom" --standalone --force 2>&1
            ...
            if [[ -f "/root/cert/${current_dom}/fullchain.pem" ]]; then
                echo -e "${GREEN}[OK] SSL cert ban gaya!${NC}"
            else
                echo -e "${RED}[ERROR] SSL nahi bana. DNS A record check karein.${NC}"
                echo -e "${YELLOW}Baad mein x-ui command -> SSL menu se bana sakte hain.${NC}"
            fi
        else
...
fix_and_diagnose() {
    while true; do
        ...
        echo -e "  SSL cert       : $([ -f \"/root/cert/${current_dom}/fullchain.pem\" ] && echo 'EXISTS OK' || echo 'MISSING')"
        echo -e "${CYAN}----------------------------------------------------${NC}"
        ...
        case $fix_opt in
            ...
            5)
                ...
                if [[ "$current_dom" == "No Domain Set" || -z "$current_dom" ]]; then
                    ...
                    echo -e "\n${BLUE}SSL cert re-issue ho raha hai for $current_dom...${NC}"
                    systemctl stop nginx 2>/dev/null
                    systemctl stop x-ui 2>/dev/null
                    if [[ ! -f /root/.acme.sh/acme.sh ]]; then
                    ...
                    if [[ -f "/root/cert/${current_dom}/fullchain.pem" ]]; then
                        ...
                        echo -e "${RED}[ERROR] SSL nahi bana. DNS check karein (Option 3).${NC}"
                    fi
                    systemctl start x-ui 2>/dev/null
                    systemctl start nginx 2>/dev/null
                fi
...
while true; do
    ...
    echo -e " 12) ${YELLOW}Fix & Diagnose Issues (Live Debug + Fix)${NC}"
