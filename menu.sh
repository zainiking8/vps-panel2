...
server {
    ...
    ssl_certificate /root/cert/${MY_DOMAIN}/fullchain.pem;
    ssl_certificate_key /root/cert/${MY_DOMAIN}/privkey.pem;
    ssl_protocols TLSv1.2 TLSv1.3;
    ...
    while true; do
        ...
        case $fix_opt in
            0)
                ...
                local current_dom=$(get_domain)
                # 1) Fix DNS
                echo -e "${YELLOW}[1/6] DNS fix...${NC}"
                rm -f /etc/resolv.conf
                echo "nameserver 8.8.8.8" > /etc/resolv.conf
                echo "nameserver 1.1.1.1" >> /etc/resolv.conf
                # 2) Fix dropbear + OpenSSH
                echo -e "${YELLOW}[2/6] SSH services fix...${NC}"
                fix_dropbear_core
                # 3) Regenerate nginx config
                echo -e "${YELLOW}[3/6] Nginx config fix...${NC}"
                apply_nginx_config
                # 4) Fix panel credentials & port
                echo -e "${YELLOW}[4/6] Panel credentials/port fix...${NC}"
                if [[ -f /usr/local/x-ui/x-ui ]]; then
                    /usr/local/x-ui/x-ui setting -username admin -password zaini123 -port 8443 2>/dev/null
                fi
                # 5) Re-issue SSL
                echo -e "${YELLOW}[5/6] SSL cert fix...${NC}"
