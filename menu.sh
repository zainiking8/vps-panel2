...
def pick_port_notls(preferred=80):
    used = get_used_ports()
    if preferred not in used:
        return preferred
    for p in [8080, 8880, 2052, 2053, 2082, 2086, 2095, 8000, 8001, 8008, 8010, 8888]:
        if p not in used:
            return p
    for p in range(20000, 25000):
        if p not in used:
            return p
    return 8080
...
def build_stream_grpc_notls(service="tunnel"):
    return {
        "network": "grpc",
        "security": "none",
        "grpcSettings": {
...
def build_stream_ws_notls(path="/tunnel"):
    return {
        "network": "ws",
        "security": "none",
        "wsSettings": {
...
def gen_vless_link(client_uuid, domain, port, transport, sni, service_or_path, remark, security="tls"):
    params = []
    if transport == "grpc":
...
def gen_vmess_link(client_uuid, domain, port, transport, sni, service_or_path, remark, security="tls"):
    obj = {
        "v": "2",
        "ps": remark,
        "add": domain,
...
def gen_trojan_link(password, domain, port, transport, sni, service_or_path, remark, security="tls"):
    params = []
    if transport == "grpc":
        params.append("type=grpc")
        params.append("serviceName=" + service_or_path)
...
def do_create(protocol, transport, domain, cert, key, port_choice, base, web_base, cookie, security="tls"):
    ...
    sec_label = "TLS" if security == "tls" else "NON-TLS"
    ...
    if protocol == "vless":
        ...
        if transport == "grpc":
            if security == "tls":
                ...
                stream = build_stream_grpc_notls(service)
            link = gen_vless_link(client_uuid, domain, port_choice, "grpc", domain, service, remark, security)
        else:
            if security == "tls":
                stream = build_stream_ws(domain, cert, key, path)
            else:
                stream = build_stream_ws_notls(path)
            link = gen_vless_link(client_uuid, domain, port_choice, "ws", domain, path, remark, security)
    elif protocol == "vmess":
        ...
        if transport == "grpc":
            if security == "tls":
                ...
                stream = build_stream_grpc_notls(service)
            link = gen_vmess_link(client_uuid, domain, port_choice, "grpc", domain, service, remark, security)
        else:
            if security == "tls":
                stream = build_stream_ws(domain, cert, key, path)
            else:
                stream = build_stream_ws_notls(path)
            link = gen_vmess_link(client_uuid, domain, port_choice, "ws", domain, path, remark, security)
        ...
        if transport == "grpc":
            if security == "tls":
                ...
                stream = build_stream_grpc_notls(service)
            link = gen_trojan_link(trojan_pass, domain, port_choice, "grpc", domain, service, remark, security)
        else:
            if security == "tls":
                stream = build_stream_ws(domain, cert, key, path)
            else:
                stream = build_stream_ws_notls(path)
            link = gen_trojan_link(trojan_pass, domain, port_choice, "ws", domain, path, remark, security)
    else:
    ...
    if ok:
        ...
        print("=" * 55)
        print(f"  Protocol  : {protocol.upper()} + {transport.upper()} + {sec_label}")
...
def main():
    if len(sys.argv) < 2:
        ...
        print("          create-vless-grpc-notls, create-vless-ws-notls,")
        print("          create-vmess-grpc-notls, create-vmess-ws-notls,")
        print("          create-trojan-grpc-notls, create-trojan-ws-notls,")
    ...
    if cmd.startswith("create-"):
        security = "tls"
        work_cmd = cmd
        if cmd.endswith("-notls"):
            security = "none"
            work_cmd = cmd[:-5]  # strip '-notls'
        ...
        if security == "tls":
            ...
            port_choice = pick_port_notls()
        sec_label = "TLS" if security == "tls" else "NON-TLS"
        print(f"\n  Selected: {protocol.upper()} + {transport.upper()} + {sec_label}")
        print(f"  Auto Port: {port_choice}")
        do_create(protocol, transport, domain, cert, key, port_choice, base, web_base, cookie, security)
    elif cmd == "list":
...
xui_account_menu() {
    ...
    while true; do
        ...
        echo -e "  3) VLESS + gRPC ${YELLOW}(Non-TLS)${NC}"
        echo -e "  4) VLESS + WebSocket ${YELLOW}(Non-TLS)${NC}"
        echo -e " ${GREEN}VMess:${NC}"
        echo -e "  5) VMess + gRPC + SSL/TLS"
        echo -e "  6) VMess + WebSocket + SSL/TLS"
        echo -e "  7) VMess + gRPC ${YELLOW}(Non-TLS)${NC}"
        echo -e "  8) VMess + WebSocket ${YELLOW}(Non-TLS)${NC}"
        echo -e " ${GREEN}Trojan:${NC}"
        echo -e "  9) Trojan + gRPC + SSL/TLS"
        echo -e " 10) Trojan + WebSocket + SSL/TLS"
        echo -e " 11) Trojan + gRPC ${YELLOW}(Non-TLS)${NC}"
        echo -e " 12) Trojan + WebSocket ${YELLOW}(Non-TLS)${NC}"
        echo -e "${CYAN}--------------------${NC}"
        echo -e " 13) List All X-UI Accounts"
        echo -e " 14) Delete X-UI Account (by ID)"
        echo -e " 15) Re-Configure Panel Access"
        echo -e " 16) Back to Main Menu"
        echo -e "${CYAN}====================================================${NC}"
        read -rp "Select [1-16]: " x_opt

        case $x_opt in
            1) python3 /usr/local/bin/xui-helper.py create-vless-grpc ;;
            2) python3 /usr/local/bin/xui-helper.py create-vless-ws ;;
            3) python3 /usr/local/bin/xui-helper.py create-vless-grpc-notls ;;
            4) python3 /usr/local/bin/xui-helper.py create-vless-ws-notls ;;
            5) python3 /usr/local/bin/xui-helper.py create-vmess-grpc ;;
            6) python3 /usr/local/bin/xui-helper.py create-vmess-ws ;;
            7) python3 /usr/local/bin/xui-helper.py create-vmess-grpc-notls ;;
            8) python3 /usr/local/bin/xui-helper.py create-vmess-ws-notls ;;
            9) python3 /usr/local/bin/xui-helper.py create-trojan-grpc ;;
            10) python3 /usr/local/bin/xui-helper.py create-trojan-ws ;;
            11) python3 /usr/local/bin/xui-helper.py create-trojan-grpc-notls ;;
            12) python3 /usr/local/bin/xui-helper.py create-trojan-ws-notls ;;
            13) python3 /usr/local/bin/xui-helper.py list ;;
            14)
